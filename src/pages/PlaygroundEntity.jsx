...
export default function PlaygroundEntity() {
  ...
  const runTests = async () => {
    ...
    for (const test of tests) {
      try {
        const { name, description, category, schema } = template;
        let result = false;
  ...
  const generateAISuggestions = async () => {
    ...
    try {
      const result = await base44.integrations.Core.InvokeLLM({
        prompt: `Analyze this entity template and suggest improvements:

Entity Name: ${template.name}
Description: ${template.description || "None"}
Category: ${template.category}
Fields: ${JSON.stringify(template.schema?.properties || {}, null, 2)}
...
- Best practices for this type of entity
          ...
          <CardContent>
            {template ? (
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-500 mb-2">Fields ({Object.keys(template.schema?.properties || {}).length})</h4>
                  <div className="space-y-2">
                    {Object.entries(template.schema?.properties || {}).map(([key, value]) => (
                ...
                {template.schema?.required?.length > 0 && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-500 mb-2">Required Fields</h4>
                    <div className="flex flex-wrap gap-1">
                      {template.schema.required.map(f => (
                        <Badge key={f} className="bg-red-100 text-red-800">{f}</Badge>
                      ))}
                    </div>
                  </div>
                )}
