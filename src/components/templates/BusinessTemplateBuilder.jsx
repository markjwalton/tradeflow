        ...
        <div className="space-y-6 pb-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {tags.map((tag) => (
                <Badge key={tag} variant="secondary" className="gap-1">{tag}<button onClick={() => setTags(tags.filter(t => t !== tag))} className="hover:text-destructive">×</button></Badge>
              ))}
            </div>
          </div>
                ...
                {entityRelationships.length > 0 && (
                  <div className="mt-3 space-y-1">
                    {entityRelationships.map((rel, idx) => (
                      <div key={idx} className="text-xs bg-card rounded px-2 py-1 flex items-center gap-2">
      ...
      {showLibrary && availableItems.length > 0 && (
        <div className="grid grid-cols-2 gap-2 p-3 bg-muted rounded-lg max-h-48 overflow-y-auto">
          {availableItems.map((item) => (
            <Card key={item.id} className="cursor-pointer hover:bg-primary/10 transition-colors" onClick={() => { onAdd(item); setShowLibrary(false); }}>
              ...
              <CardContent className="p-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => onRemove(idx)}>
