      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="general" className="mt-4">
          ...
          <TabsContent value="general" className="space-y-4 mt-4">
            <div>
              <label className="text-sm font-medium text-foreground">Visible Tabs</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-foreground">Days Until Auto-Archive</label>
                <Input
                  type="number"
                  ...
                  className="mt-1"
                />
                <p className="text-xs text-muted-foreground mt-1">Completed items archive after this many days</p>
              </div>
              <div>
                <label className="text-sm font-medium text-foreground">Default Items Per Page</label>
                <Select 
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label className="text-sm font-medium text-foreground">Allow Delete</label>
            </div>

            <div>
              <label className="text-sm font-medium text-foreground">Page Size Options</label>
          ...
          <TabsContent value="categories" className="space-y-4 mt-4">
            <div className="space-y-2">
              {formData.categories?.map((cat, index) => (
                <div key={index} className="flex items-center gap-2 bg-background p-2 rounded-lg border border-border">
                  <GripVertical className="h-4 w-4 text-muted-foreground" />
                  <span className="text-sm font-mono text-muted-foreground w-32">{cat.value}</span>
                  <span className="flex-1 text-foreground">{cat.label}</span>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => removeCategory(index)}>
          ...
          <TabsContent value="statuses" className="space-y-4 mt-4">
            <div className="space-y-2">
              {formData.statuses?.map((status, index) => (
                <div key={index} className="flex items-center gap-2 bg-background p-2 rounded-lg border border-border">
                  <GripVertical className="h-4 w-4 text-muted-foreground" />
                  <span className="text-sm font-mono text-muted-foreground w-28">{status.value}</span>
                  <span className="flex-1 text-foreground">{status.label}</span>
                  ...
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => removeStatus(index)}>
                    <Trash2 className="h-3 w-3" />
                  </Button>
                </div>
              ))}
          ...
          <TabsContent value="priorities" className="space-y-4 mt-4">
            <div className="space-y-2">
              {formData.priorities?.map((pri, index) => (
                <div key={index} className="flex items-center gap-2 bg-background p-2 rounded-lg border border-border">
                  <GripVertical className="h-4 w-4 text-muted-foreground" />
                  <span className="text-sm font-mono text-muted-foreground w-24">{pri.value}</span>
                  <span className="flex-1 text-foreground">{pri.label}</span>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => removePriority(index)}>
                    <Trash2 className="h-3 w-3" />
                  </Button>
                </div>
              ))}
